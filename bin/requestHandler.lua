...
local function getUserAddress(userAddress)
  ...
  if exists == false then
    userAddress = userTable[table.getn(userTable)]
    userNumericalAddress = tostring(table.getn(userTable))
...
if info ==  "in" then
  if table.getn(destinationTable) ~= 0 then
    ...
    if failed then
      userNumericalAddress = userAssignAddress(address)
      assignDestinationTable(i, userNumericalAddress)
    end
  else
    userAssignAddress(address)
